...
// }
    ...
    try {
      ...
      if (true) {
        ...
        $mailParams = [
          ...
          'toEmail' => $attendeeEmail,
          'replyTo' => $from,
          'html' => self::getVolunteerFeedbackCollectionActivityEmailHtml($attendeeName, $eventId, $eventAttendedById, $eventCode, $eventAddress),
        ];
   ...
   *
   */
  private static function getVolunteerFeedbackCollectionActivityEmailHtml($attendeeName, $eventId, $eventAttendedById, $eventCode, $eventAddress) {
    $homeUrl = \CRM_Utils_System::baseCMSURL();
    // Construct the full URLs for the forms.
    $campOutcomeFormUrl = $homeUrl . 'events-feedback/#?Event1=' . $eventId . '&Goonj_Events_Outcome.Filled_By=' . $campAttendedById;

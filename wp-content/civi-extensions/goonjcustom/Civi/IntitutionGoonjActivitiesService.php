...
class IntitutionGoonjActivitiesService extends AutoSubscriber {
  ...
  const ENTITY_SUBTYPE_NAME = 'Institution_Goonj_Activity';
  ...
  const INSTITUTE_GOONJ_ACTIVITIES_INTENT_FB_NAME = 'afformInstitutionGoonjActivitiesIntent';
  ...
  public static function assignChapterGroupToIndividual(string $op, string $objectName, $objectId, &$objectRef) {
    ...
    $stateId = $objectRef['Institute_Goonj_Activities.State'];
    $contactId = $objectRef['Institute_Goonj_Activities.Institution_POC'];
    $organizationId = $objectRef['Institute_Goonj_Activities.Organization_Name'];
  ...
  public static function mailNotificationToMmt($op, $groupID, $entityID, &$params) {
    ...
    $collectionCamp = EckEntity::get('Collection_Camp', FALSE)
      ->addSelect('Institute_Goonj_Activities.Collection_Camp_Address', 'title')
    ...
    $campAddress = $collectionCamp['Institute_Goonj_Activities.Collection_Camp_Address'];
  ...
  public static function sendLogisticsEmail($collectionCamp) {
    try {
      ...
      $campCode = $collectionCamp['title'];
      $campOffice = $collectionCamp['Institute_Goonj_Activities.Goonj_Office'];
      $campAddress = $collectionCamp['Institute_Goonj_Activities.Collection_Camp_Address'];
      ...
      $institutionPOCId = $collectionCamp['Institute_Goonj_Activities.Institution_POC'];
      $coordinatingPOCId = $collectionCamp['Institute_Goonj_Activities.Coordinating_POC'];
      $organizationId = $collectionCamp['Institute_Goonj_Activities.Organization_Name'];

      $startDate = new \DateTime($collectionCamp['Institute_Goonj_Activities.Collections_will_start_on_Date_']);
  ...
  private static function findStateField(array $array) {
    $institutionGoonjActivitiesStateField = CustomField::get(FALSE)
      ->addSelect('id')
      ->addWhere('name', '=', 'state')
      ->addWhere('custom_group_id:name', '=', 'Institute_Goonj_Activities')
      ->execute()
      ->first();
  ...
  public static function assignCoordinatorByRelationshipType($stateOfficeId, $registrationType, $collectionCampId) {
    ...
    $registrationCategorySelection = $registrationType['Institute_Goonj_Activities.You_wish_to_register_as:name'];
    ...
    $res = EckEntity::update('Collection_Camp', FALSE)
      ->addValue('Institute_Goonj_Activities.Coordinating_Urban_Poc', $coordinator['contact_id_a'])
      ->addWhere('id', '=', $collectionCampId)
      ->execute();
  ...
  public static function setOfficeDetails($op, $groupID, $entityID, &$params) {
    ...
    //   ->addSelect('Institute_Goonj_Activities.Will_your_collection_drive_be_open_for_general_public_as_well')
    //   ->addWhere('id', '=', $institutionCollectionCampId)
    //   ->execute()->single();
    // $isPublicDriveOpen = $institutionCollectionCampData['Institute_Goonj_Activities.Will_your_collection_drive_be_open_for_general_public_as_well'];
    if (!$stateId) {
    ...
    EckEntity::update('Collection_Camp', FALSE)
      ->addValue('Institute_Goonj_Activities.Goonj_Office', $stateOfficeId)
      ->addWhere('id', '=', $institutionCollectionCampId)
      ->execute();

    $registrationType = EckEntity::get('Collection_Camp', FALSE)
      ->addSelect('Institute_Goonj_Activities.You_wish_to_register_as:name')
  ...
  public static function setGoonjActivitiesAddress(AfformSubmitEvent $event) {
    ...
    // \Civi::log()->info('formName',['formName'=>$formName,'afform'=>$afform ]);
    if ($formName !== self::INSTITUTE_GOONJ_ACTIVITIES_INTENT_FB_NAME) {
      return;
    }

    $entityType = $event->getEntityType();
    // \Civi::log()->info('entityType', ['entityType'=>$entityType]);
    if ($entityType !== 'Eck_Collection_Camp') {
      return;
    }

    $records = $event->records;
    // \Civi::log()->info('records', ['entityType'=>$records]);
    foreach ($records as $record) {
      $fields = $record['fields'];

      self::$instituteGoonjActivitiesAddress = [
        'location_type_id' => 3,
        'state_province_id' => $fields['Institute_Goonj_Activities.State'],
      // India.
        'country_id' => 1101,
        'street_address' => $fields['Institute_Goonj_Activities.Where_do_you_wish_to_organise_the_activity_'],
        'city' => $fields['Institute_Goonj_Activities.City'],
        'postal_code' => $fields['Institute_Goonj_Activities.Postal_Code'],
        'is_primary' => 1,
      ];
      self::$institutePocAddress = [
        'location_type_id' => 3,
        'state_province_id' => $fields['Institute_Goonj_Activities.State'],
      // India.
        'country_id' => 1101,
      ...
      \Civi::log()->info('instituteGoonjActivitiesA12312312ddress', ['instit21212uteGoonjActivitiesAddress' => self::$instituteGoonjActivitiesAddress, 'institutePocAddress' => self::$institutePocAddress]);
    }
  }
  ...
  public static function setActivitiesVolunteersAddress(AfformSubmitEvent $event) {
    ...
    foreach ($event->records as $index => $contact) {
      \Civi::log()->info('event2', ['event2' => $event, 'contact' => $contact]);
      if (empty($contact['fields'])) {
        continue;
      }

      // Ensure 'joins' and 'Address' keys exist as arrays.
      $joins = $event->records[$index]['joins']['Address'] ?? [];

      // Check if the address is already in the array before adding it.
      if (!in_array(self::$instituteGoonjActivitiesAddress, $joins)) {
        \Civi::log()->info('joined', ['event2' => $joins, 'contacinstituteGoonjActivitiesAddresst' => $instituteGoonjActivitiesAddress]);
        // Add if not present.
        $joins[] = self::$instituteGoonjActivitiesAddress;
      }

      // Update the 'joins' key.
      \Civi::log()->info('joined11', ['event2' => $joins, 'contacinstituteGoonjActivitiesAddresst' => $instituteGoonjActivitiesAddress]);
      $event->records[$index]['joins']['Address'] = $joins;
      \Civi::log()->info('joined22', ['event2' => $joins, 'contacinstituteGoonjActivitiesAddresst' => $instituteGoonjActivitiesAddress]);
    }
  ...
  public static function createActivityForGoonjActivityCollectionCamp(string $op, string $objectName, $objectId, &$objectRef) {
    ...
    $collectionCamp = EckEntity::get('Collection_Camp', FALSE)
      ->addSelect('Collection_Camp_Core_Details.Status', 'title')
      ->addWhere('id', '=', $objectId)
      ->execute()->single();
    ...
    $activities = $objectRef['Institute_Goonj_Activities.How_do_you_want_to_engage_with_Goonj_'];
    $startDate = $objectRef['Institute_Goonj_Activities.Date_for_conducting_the_activity_'];
    $initiator = $objectRef['Collection_Camp_Core_Details.Contact_Id']?? NULL;

    foreach ($activities as $activityName) {
      // Check if the activity is 'Others'.
      if ($activityName == 'Other') {
        $otherActivity = $objectRef['Institute_Goonj_Activities.Other_Activity_Details'] ?? '';
      ...
      $optionValue = OptionValue::get(TRUE)
        ...
        ->addWhere('name', '=', 'Institute_Goonj_Activities')
        ->execute()->single();

...
class CollectionCampService extends AutoSubscriber {
  ...
  */
  public static function sendEmailToOrganizerAfterAcknowledgement(string $op, string $objectName, int $objectId, &$objectRef) {
    ...
    (
        $objectRef->afform_name !== 'afformFrontFacingAcknowledgementFormForLogistics' &&
        $objectRef->afform_name !== 'afformAcknowledgementFormForLogistics'
    )
    ) {
      return;
    }
    ...
    $collectionCampId = $data['Eck_Collection_Source_Vehicle_Dispatch1'][0]['fields']['Camp_Vehicle_Dispatch.Collection_Camp'] ?? NULL;
    ...
    $collectionCampDispatch = EckEntity::get('Collection_Source_Vehicle_Dispatch', FALSE)
      ->addSelect('Camp_Vehicle_Dispatch.Date_Time_of_Dispatch', 'Camp_Vehicle_Dispatch.To_which_PU_Center_material_is_being_sent.display_name')
      ->addWhere('id', '=', $ackId)
      ->execute()->first();
  ...
  public static function getAcknowledgementEmailHtml($organizerName, $materialDescription, $collectionCampDate, $goonjOfficeName, $venue, $city) {
    ...
    <p>We are happy to share that we have received <strong>{$materialDescription}</strong> on <strong>{$collectionCampDate}</strong> at <strong>{$goonjOfficeName}</strong> from <strong>{$venue}</strong>, <strong>{$city}</strong>.</p>
  ...
  public static function reGenerateCollectionCampQr(string $op, string $objectName, int $objectId, &$objectRef) {
    ...
    try {
      ...
      if ($subtype !== 'Collection_Camp' || $status !== 'authorized' || $collectionCampQr !== NULL) {
        return;
      }

      self::generateCollectionCampQrCode($collectionCampId, $collectionCamp);

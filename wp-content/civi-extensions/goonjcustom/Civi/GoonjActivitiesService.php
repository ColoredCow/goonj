...
class GoonjActivitiesService extends AutoSubscriber {
  ...
  public static function goonjActivitiesTabset($tabsetName, &$tabs, $context) {
    ...
    $tabConfigs = [
      'edit' => [
        ...
        'permissions' => ['goonj_chapter_admin', 'urbanops', 'urban_ops_admin', 'urban_ops_and_accounts_chapter_team', 'mmt_and_accounts_chapter_team', 'project_ho_and_accounts', 'project_team_ho', 'njpc_ho_team'],
      ],
      'activities' => [
        'title' => ts('Activities'),
        ...
        'permissions' => ['goonj_chapter_admin', 'urbanops', 'urban_ops_admin', 'urban_ops_and_accounts_chapter_team', 'project_ho_and_accounts', 'project_team_ho', 'njpc_ho_team'],
      ],
      'logistics' => [
        'title' => ts('Logistics'),
  ...
  public static function generateGoonjActivitiesQr(string $op, string $objectName, $objectId, &$objectRef) {
    ...
    $currentStatus = $currentCollectionCamp['Collection_Camp_Core_Details.Status'];
    $collectionCampId = $currentCollectionCamp['id'];
    $goonjActivityQr = $currentCollectionCamp['Collection_Camp_QR_Code.QR_Code'];

    if ($goonjActivityQr !== NULL) {
      self::generateGoonjActivitiesQrCode($collectionCampId, $objectRef);
      return;
    }
   ...
   *
   */
  private static function generateGoonjActivitiesQrCode($id, $objectRef) {

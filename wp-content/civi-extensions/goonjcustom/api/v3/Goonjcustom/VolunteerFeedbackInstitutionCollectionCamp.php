...
/**
 * Goonjcustom.VolunteerFeedbackInstitutionCollectionCampCron API specification (optional)
 ...
 * @return void
 */
function _civicrm_api3_goonjcustom_volunteer_feedback_institution_collection_camp_cron_spec(&$spec) {
  // There are no parameters for the Goonjcustom cron.
}

/**
 * Goonjcustom.InstitutionCollectionCampCron API.
 *
 * @param array $params
 *
 ...
 */
function civicrm_api3_goonjcustom_volunteer_feedback_institution_collection_camp_cron($params) {
  $returnValues = [];
  ...
  foreach ($collectionCamps as $camp) {
    try {
      ...
      $institutionCollectionCampId = $camp['id'];
      ...
      if (!$feedbackEmailSent && $endDateFormatted <= $todayFormatted) {
        $mailParams = [
          ...
          'replyTo' => $from,
          'html' => goonjcustom_volunteer_feedback_institution_collection_camp_email_html($organizingContactName, $institutionCollectionCampId, $campAddress),
        ];
      ...
      \Civi::log()->info("Error processing camp ID $institutionCollectionCampId: " . $e->getMessage());
    }
  }

  return civicrm_api3_create_success($returnValues, $params, 'Goonjcustom', 'volunteer_feedback_institution_collection_camp_cron');
}

/**
 *
 */
function goonjcustom_volunteer_feedback_institution_collection_camp_email_html($organizingContactName, $institutionCollectionCampId, $campAddress) {
  $homeUrl = \CRM_Utils_System::baseCMSURL();

  // URL for the volunteer feedback form.
  $campVolunteerFeedback = $homeUrl . 'volunteer-institution-camp-feedback/#?Eck_Collection_Camp1=' . $institutionCollectionCampId;
